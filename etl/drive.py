...
try:
    import gdown  # robust Google Drive downloader (handles confirmation pages)
except Exception:
    gdown = None
...
def drive_share_to_download(url: str) -> str:
    """Legacy helper: convert a Drive share link to a 'uc?export=download' URL."""
...
def download_to(path: Path, url: str, chunk=2**20) -> Path:
    """Download a URL to a specific path. Uses gdown if it's a Drive link."""
    fid = extract_id(url or "")
    if fid:
...
def download_drive_file(url: str, dest_dir: Path) -> Path:
    """Download a Google Drive file by ID into 'dest_dir' preserving the original filename."""
    if gdown is None:
    ...
    if out is None:
        raise RuntimeError("gdown failed; check Drive permissions & link.")
    return Path(out)
